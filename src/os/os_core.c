...
INT8U  OS_EventTaskRdy (OS_EVENT *pevent, void *pmsg, INT8U msk, INT8U pend_stat)
{
	...
	#else
	y	  = pgm_read_byte(OSUnMapTbl + pevent->OSEventGrp);
	#endif
    bity  = (INT8U)(1 << y);
	#ifndef ALL_IN_ROM
	...
	#else
		y = pgm_read_byte(OSUnMapTbl + (pevent->OSEventGrp & 0xFF));
	#endif
    } else {
	#ifndef ALL_IN_ROM
        y = OSUnMapTbl[(pevent->OSEventGrp >> 8) & 0xFF] + 8;
	#else
		y = pgm_read_byte(OSUnMapTbl + ((pevent->OSEventGrp >> 8) & 0xFF)) + 8;
	#endif
    }
    bity = (INT16U)(1 << y);
	...
	#else
		x = pgm_read_byte(OSUnMapTbl + (*ptbl & 0xFF));
	#endif
    } else {
	#ifndef ALL_IN_ROM
        x = OSUnMapTbl[(*ptbl >> 8) & 0xFF] + 8;
	#else
		x = pgm_read_byte(OSUnMapTbl + ((*ptbl >> 8) & 0xFF)) + 8;
	#endif
    }
    bitx = (INT16U)(1 << x);
...
static  void  OS_SchedNew (void)
{
	...
	#else
		y = pgm_read_byte(OSUnMapTbl + (OSRdyGrp & 0xFF));
	#endif
    } else {
	#ifndef ALL_IN_ROM
        y = OSUnMapTbl[(OSRdyGrp >> 8) & 0xFF] + 8;
	#else
		y = pgm_read_byte(OSUnMapTbl + ((OSRdyGrp >> 8) & 0xFF) + 8;
    #endif
	}
    ptbl = &OSRdyTbl[y];
    if ((*ptbl & 0xFF) != 0) {
	...
	#else
		OSPrioHighRdy = (INT8U)((y << 4)) + pgm_read_byte(OSUnMapTbl + (*ptbl & 0xFF));
	#endif
    } else {
	#ifndef ALL_IN_ROM
        OSPrioHighRdy = (INT8U)((y << 4) + OSUnMapTbl[(*ptbl >> 8) & 0xFF] + 8);
	#else
		OSPrioHighRdy = (INT8U)((y << 4) + pgm_read_byte(OSUnMapTbl + ((*ptbl >> 8) & 0xFF)) + 8);
	#endif
    }
#endif
}
